...
![旁路由控制台运行总览](docs/screenshots/web-overview-real.jpg)

> 实际登录后的运行总览界面：展示核心服务、DNS 隐私策略、透明代理流量、系统资源和快捷维护入口。截图不包含订阅、节点凭据、密码、Token、客户端身份或日志内容。控制台地址为 `http://旁路由IP:Web端口`。
